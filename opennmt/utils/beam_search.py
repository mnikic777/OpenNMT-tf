...
def beam_search(symbols_to_logits_fn,
  ...
  Args:
    ...
    return_states: a boolean - return dict (possibly nested) of decoding states.
  Returns:
    Tuple of
    (decoded beams [batch_size, beam_size, decode_length]
     decoding probabilities [batch_size, beam_size]) or
     (decoded beams [batch_size, beam_size, decode_length]
     decoding probabilities [batch_size, beam_size], final_states)
     if :obj:`return_states` is ``True``.
  """
  ...
  (_, alive_seq, alive_log_probs, finished_seq, finished_scores,
   finished_flags, final_states) = tf.while_loop(
       _is_finished,
       inner_loop, [
  ...
  if return_states:
    return finished_seq, finished_scores, final_states
